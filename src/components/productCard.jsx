...
export default function ProductCard(props) {
    ...
    // Optional chaining පාවිච්චි කර රූප ඇති දැයි පරීක්ෂා කිරීම
    const hasMultipleImages = product.image && product.image.length > 1;

    return (
        <div className="w-[300px] h-[450px] shadow-2xl shadow-black/20 rounded-[40px] m-4 overflow-hidden bg-white flex flex-col group relative cursor-pointer border border-gray-100">
            
            {/* Image Section */}
            <div className="w-full h-[300px] relative bg-gray-50 overflow-hidden">
                {/* දෙවන රූපය (Hover කළ විට පෙනෙන) */}
                {hasMultipleImages && (
                    <img 
                        src={product.image[1]} 
                ...
                {/* ප්‍රධාන රූපය */}
                <img 
                    src={product.image?.[0] || "https://via.placeholder.com/300x300?text=No+Image"} 
            ...
            <div className="p-5 flex flex-col flex-grow justify-between">
                <div>
                    <h1 className="font-bold text-lg truncate text-gray-800" title={product.name}>
                        {product.name}
                    </h1>
                    
                    <div className="flex flex-col mt-1">
                        {/* Labeled price එක විකුණුම් මිලට වඩා වැඩි නම් පමණක් පෙන්වන්න */}
                        ...
                        <h2 className="text-blue-600 font-extrabold text-xl">
                            LKR {Number(product.price || 0).toLocaleString(undefined, {minimumFractionDigits: 2})}
                        </h2>
                    </div>
                </div>
            </div>
